# CLAUDE.md
<!-- Generated by Kodizm AI · Last updated: 2026-04-11 -->
...
| Project | Purpose |
|---------|---------|
| **Kodizm API** | Laravel backend — source of truth for API response shapes, controllers, resources |
| **magic** | Core framework — Http, Auth, Vault, MagicRoute, MagicStateMixin, Config |
| **magic_starter** | App scaffold — auth screens, layout, PageHeader/Card patterns. **Reference for view standards** |
| **Wind UI** | Tailwind-for-Flutter — WDiv, WText, WIcon, WSpacer, WAnchor, WFormInput |
...
## Agent Context (inject into all subagent prompts — they do NOT inherit CLAUDE.md)
...
2. i18n via `trans('section.key')` from `assets/lang/en.json`. Params: `trans('key', {'param': value.toString()})`, placeholder syntax `:param`
3. Atomic design — atoms/ molecules/ organisms/ templates/ in `lib/resources/widgets/`. Views in `lib/resources/views/`
4. Layout — Root `WDiv(className: 'p-4 lg:p-6 flex flex-col gap-6')` + PageHeader + SectionCard. No max-w-*, no SingleChildScrollView
5. DESIGN.md tokens — No raw Color(), BoxDecoration, TextStyle. WDiv with `flex items-center justify-center` MUST include `w-full`
6. Magic framework — Http, MagicRoute, ChangeNotifier+MagicStateMixin, Vault, Config, Log. Never raw Dio, GoRouter, print()
